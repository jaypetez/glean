...
- Telegram bot *interaction* (replying to user commands beyond `/start` ack). Inbound commands are out of scope until future release.
- Embedding-based semantic dedup. v1.3.0 dedups by canonical URL + content-hash only.
...
#### Concrete source plugins (v1.3.0)
| Type | Implementation |
|---|---|
...
- `/metrics` (optional, future release) — Prometheus counters: `runs_total{feed,outcome}`, `items_sent_total{feed}`, `llm_calls_total{provider}`, `llm_latency_seconds`.
...
## 15. Open questions deferred to future release
