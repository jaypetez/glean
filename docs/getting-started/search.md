...
Generate a strong per-instance secret before enabling the optional SearXNG
service:
...
The compose service validates this value at startup. If `SEARXNG_SECRET` is
missing, shorter than 32 hex characters, or contains non-hex characters, the
SearXNG container exits with an error instead of starting with a weak default.

The SearXNG config directory is mounted read-only (`./searxng-config:/etc/searxng:ro`).
Keep it read-only when possible so a compromised container cannot rewrite its
own settings. The service sets `FORCE_OWNERSHIP=false` to avoid startup writes
that would otherwise conflict with the read-only bind mount.

**Threat model:** SearXNG is intended to be internal-network-only and is not a
tenant-isolation boundary. Do not expose it to the public internet. If the
SearXNG container is compromised, treat it as a window into your search history
and any search queries generated by glean.
