# 01 — Web search → local LLM → file sink
...
- Writes the rendered digest to `./data/digests/web-search.md` (no Telegram / Discord / Slack required).
...
| Sink | `file` → `/data/digests/web-search.md` (markdown, appended) |
...
This forces a one-off run and writes to `data/digests/web-search.md`. The next scheduled tick still fires on the hour.
